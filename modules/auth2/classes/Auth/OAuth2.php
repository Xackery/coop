...
*/
			...
			else
			{
				...
				{
					// Load the role
					$roles = DB::query(Database::SELECT, 'SELECT * FROM role where name = :role')->param('role', $role)->execute(); 
					if (!count($roles))
						return FALSE;
				}
				else
	...
	protected function _login($email, $password, $remember)
	{
		...
		$role = DB::select()->from('role')->where('role', '=', 'login')->execute();

		if (count($role) AND $user->password === $password)
		{
			if ($remember === TRUE)
			{
				// Token data
				...
				DB::insert('user_token', array('user_id', 'expires', 'user_agent'))->values($data);
